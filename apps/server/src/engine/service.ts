...
import { auditLog, clearRunState } from "../scheduler/run-state.ts";
import { getScheduledTask, grantOf, recordTaskRun } from "../scheduler/scheduled-tasks.ts";
...
export class AgentService {
  ...
  private async execute(
    ...
    // New scheduler subsystem: rows in the scheduled_tasks table carry
    // their grant in input.scheduledTaskId and run through the
    // capability-aware runner below. Chat-created schedules keep the
    // original runSchedule path.
    if (task.kind === "scheduled" && task.input.scheduledTaskId) {
      try {
        return await this.runCapabilityScheduledTask(owner, task, context);
      } catch (error) {
        if (error instanceof LostLeaseError || context.signal.aborted) throw error;
        await context.guard();
        const failures = Number(task.state.failures ?? 0) + 1;
        const detail = error instanceof Error ? error.message : "Scheduled task run failed";
        const nextRunAt = new Date(Date.now() + Math.min(60, 2 ** failures) * 60000).toISOString();
        await context.event(
          "error",
          failures >= 5 ? "Scheduled task paused after repeated failures" : "Scheduled task run failed; retrying",
          detail,
        );
        return {
          status: failures >= 5 ? "paused" : "scheduled",
          ...
          state: {
            ...
            notice: {
              ...
              body: detail,
              key: `schedtask-error:${task.id}:${failures >= 5 ? "paused" : "retry"}`,
            },
          },
        };
      }
  ...
  /**
   * Execute one due scheduled_tasks row (new scheduler subsystem). The
   * SchedulerService enqueues kind="scheduled" tasks carrying
   * input.scheduledTaskId; this mints a run id, audits run start/finish,
   * injects the row's grant into executeModelTask, then advances the
   * row's cron slot (or consumes the one-shot).
   */
  private async runCapabilityScheduledTask(
    owner: string,
    task: AgentTask,
    context: TaskContext,
  ): Promise<Partial<AgentTask>> {
    const rowId = String(task.input.scheduledTaskId ?? "");
    const row = await getScheduledTask(this.db, owner, rowId);
    if (!row || !row.enabled || row.status === "fired") {
      return { status: "paused", state: { ...task.state, notice: null } };
    }
    await context.guard();
    await context.event("step", "Running scheduled task", row.name);
    ...
    try {
      const outcome = await executeModelTask(this, owner, task, context, {
        scheduled: { runId, grant: grantOf(row) },
      });
      const at = date();
      const updated = await recordTaskRun(this.db, owner, row.id, new Date(at));
      await auditLog(this.db, {
        runId,
        owner,
        scheduleId: row.id,
      ...
      if (outcome.status === "succeeded") {
        ...
        await this.notify(
          ...
          summary,
          task.id,
          `schedtask-run:${row.id}:${updated?.runCount ?? 0}`,
        );
      } else {
        // Untrusted target (or another approval-gated call): the worker has
        // no approval card of its own, so tell the owner what needs their
        // decision. Approving means adding the target to the task's allowed
        // targets; the backup gate still cannot be bypassed.
        const text = `${typeof outcome.result === "string" ? outcome.result : ""} ${
          typeof outcome.error === "string" ? outcome.error : ""
        }`;
        if (/requires owner approval/i.test(text)) {
          await this.notify(
            owner,
            `Scheduled task needs your approval: ${row.name}`,
            "The run tried to reach a target that is not in this task's allowed list. " +
              "Add the target to the task (chat: update the scheduled task's targets) and it will work on the next run. " +
              `Detail: ${text.slice(0, 1500)}`,
            task.id,
            `schedtask-approval:${row.id}:${runId}`,
          );
        }
      }
      ...
      // Recurring: this slot's task finishes here. The scheduler tick enqueues
      // a fresh per-slot AgentTask when the row is next due (the idempotency
      // key is per slot). Leaving this record "scheduled" would double-fire
      // the next slot: the worker would claim both this task and the new one.
      const terminalStatus =
        outcome.status === "succeeded" || outcome.status === "waiting_approval"
          ? "succeeded"
          : "failed";
      return {
        ...
        ...(terminalStatus === "failed"
          ? { error: `Scheduled run ended without a result (status: ${outcome.status}).` }
          : {}),
        state: { ...task.state, ...outcome.state, failures: 0, notice: null },
      };
    } finally {
